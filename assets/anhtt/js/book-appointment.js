(function (global, factory) {
        ...
        var options = _jquery2.default.extend(true, {}, defaults, {
            ...
            templates: {
                buttons: function buttons() {
                    ...
                    var html = '<div class="winzard-buttons mt-20">' + '<a class="btn btn-default mr-20" href="#exampleWizardFormContainer" data-wizard="back" role="button">Cancel</a>' + '<a class="btn btn-finish float-right" href="#exampleWizardFormContainer" data-wizard="finish" role="button">Submit</a>' + '<a class="btn btn-primary btn-next float-right" href="#exampleWizardFormContainer" data-wizard="next" id="btn-next" role="button">Next</a>' + '</div>';
                    return html;
                }
            },
            onNext: function(e) {
                if (e.index === 1) { // step #exampleGettingOne
                    $('#choose-nailtech').css('display', 'block');
                    $('#choose-nailtech').slick({
                        dots: false,
                        infinite: false,
                        speed: 500,
                        slidesToShow: 4,
    ...
    (function () {
        ...
        serviceSelect.on('change', function () {
            ...
            if (this.checked)
                thisCheckbox.parent().addClass('active');
            else
                thisCheckbox.parent().removeClass('active');
        })
    })();
    (function () {
        $(document).on('click', '.open-modal', function () {
            $('#mdl-request-nailtech').modal('show');
        });
    })();
